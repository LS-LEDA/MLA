<template>
  <div
    class='flex flex-col space-y-10 lg:space-y-0 lg:flex-row w-full h-full lg:divide-x'
  >
    <!-- Application information -->
    <div
      class='flex flex-col w-full lg:w-2/6 h-full place-items-center justify-center'
    >
      <!-- Application logo -->
      <img :src='mla_logo' alt='MLA logo' class='h-32 w-32 my-5' />
      ...
      <div class='flex flex-col place-items-center'>
        <div v-for='(link, index) in external_links' :key='index'>
          <a
            ...
            :href='link.link'
            class='flex place-items-center hover:underline underline-offset-2'
            target='_blank'
          >
            {{ link.name }}
            <SvgIcon
    ...
    <div class='flex flex-col w-full lg:w-4/6 h-full space-y-2 lg:px-5'>
      <div class='flex font-bold text-2xl self-center'>
        {{ $t('settings.about.contributors') }}
      </div>
      <div
        class='flex w-full h-full sm:grid md:grid-cols-2 md:auto-rows-min gap-5 overflow-y-scroll'
      >
        <ContributorBadge
