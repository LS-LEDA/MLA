<template>
  <div
    :class="nav_state ? null : 'justify-center items-center'"
    class='flex flex-row py-2 mx-2 my-5 h-20'
  >
    ...
    <div class='self-center'>
      ...
      >
        <img
          alt='MLA Logo'
          class='max-h-16 rounded-md origin-center transform transition duration-500'
          src='/assets/mla_logo.png'
        />
      </router-link>
    </div>
    <!-- Application name -->
    <div
      v-if='nav_state'
      class='flex mx-4 font-bold text-2xl self-center hover:cursor-pointer'
    >
      <router-link
        :to='imported_data'
        @click="redirectionAlert(this.$t('errors.import_needed'))"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      imported_data() {
        if (
          this.appStore.imported_data.moodle_logs &&
          this.appStore.imported_data.forum_logs
        ) {
