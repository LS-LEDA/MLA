<template>
  <div class='flex flex-col m-2 p-5 bg-gray-100 rounded-lg'>
    <div class='flex w-full items-center justify-between'>
      <!-- Calendar Month & Year -->
      <span class='flex text-xl font-bold' tabindex='0'>
        {{ formatter.format(date) }} {{ date.getFullYear() }}
      </span>
      <!-- Calendar buttons -->
      <div class='flex gap-x-2'>
        <button
          class='bg-primary dark:bg-dark_primary rounded-md hover:bg-primary_variant dark:hover:bg-dark_primary_variant'
        >
          <SvgIcon :path='left_icon' type='mdi' @click='left_calendar' />
        </button>
        <button
          class='bg-primary dark:bg-dark_primary rounded-md hover:bg-primary_variant dark:hover:bg-dark_primary_variant'
        >
          <SvgIcon :path='right_icon' type='mdi' @click='right_calendar' />
        </button>
      </div>
    </div>
    <!-- Calendar dates -->
      ...
      <table class='w-full'>
        ...
        <tr v-for='(weeks, week_index) in formatted_days' :key='week_index'>
          <td v-for='(day, day_index) in weeks' :key='day_index'>
            <div
              class='p-0.5 lg:p-1 cursor-pointer flex w-full justify-center hover:bg-primary dark:hover:bg-dark_primary hover:rounded-full'
              @click='selectDate(week_index, day_index, $event)'
              v-html='day'
            ></div>
          </td>
        </tr>
    ...
    methods: {
      render_calendar: function() {
        ...
        for (let i = first_day_index; i > 0; i--) {
          this.days.push(
            `<p class="text-base text-gray-400">${prev_last_day - i + 1}</div>`,
          );
        }
        ...
        if (this.chunks === 6)
          for (let i = 0; i < 7; i++) {
            this.formatted_days[5][
              i
              ] = `<p class="text-base text-black"></div>`;
          }
      ...
      selectDate: function(week_index, day_index, selected_day) {
        ...
        if (
          (week_index <= 1 && selected_day.target.innerText > 20) ||
